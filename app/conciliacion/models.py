"""
Modelos de base de datos para el módulo de conciliación bancaria avanzada
"""
...
class ArchivoBancario(Base):
    ...
    resultados_conciliacion = relationship("ResultadoConciliacion", back_populates="archivo_bancario")
...
class ResultadoConciliacion(Base):
    """
    Modelo para estadísticas y resultados de procesos de conciliación
    
    Almacena métricas detalladas de cada proceso de conciliación ejecutado
    """
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    empresa_id = Column(Integer, ForeignKey("empresas_contribuyentes.id", ondelete="CASCADE"), nullable=False)
    archivo_bancario_id = Column(Integer, ForeignKey("archivos_bancarios.id", ondelete="SET NULL"))
    
    # Parámetros del proceso
    fecha_proceso = Column(DateTime, nullable=False, default=func.current_timestamp())
    periodo_inicio = Column(DateTime, nullable=False)
    periodo_fin = Column(DateTime, nullable=False)
    
    # Estadísticas generales
    total_movimientos_bancarios = Column(Integer, default=0)
    total_cfdis_periodo = Column(Integer, default=0)
    
    # Estadísticas de conciliación
    movimientos_conciliados = Column(Integer, default=0)
    movimientos_pendientes = Column(Integer, default=0)
    movimientos_descartados = Column(Integer, default=0)
    movimientos_manuales = Column(Integer, default=0)
    
    # Estadísticas por método
    conciliados_exacto = Column(Integer, default=0)
    conciliados_referencia = Column(Integer, default=0)
    conciliados_aproximado = Column(Integer, default=0)
    conciliados_complemento_ppd = Column(Integer, default=0)
    conciliados_heuristica = Column(Integer, default=0)
    conciliados_ml_patron = Column(Integer, default=0)
    
    # Montos
    monto_total_conciliado = Column(DECIMAL(16, 2), default=0)
    monto_total_pendiente = Column(DECIMAL(16, 2), default=0)
    
    # Métricas de calidad
    nivel_confianza_promedio = Column(DECIMAL(3, 2))
    tiempo_procesamiento_segundos = Column(Integer)
    
    # Alertas y observaciones
    alertas_criticas = Column(JSON)  # Lista de alertas importantes
    sugerencias = Column(JSON)  # Sugerencias para mejorar conciliación
    
    # Datos adicionales
    configuracion_utilizada = Column(JSON)  # Parámetros usados en la conciliación
    errores_proceso = Column(JSON)  # Errores durante el proceso
    
    # Relaciones
    empresa = relationship("EmpresaContribuyente", back_populates="resultados_conciliacion")
    archivo_bancario = relationship("ArchivoBancario", back_populates="resultados_conciliacion")
    ...
    def __repr__(self):
        return f"<ResultadoConciliacion(id={self.id}, empresa_id={self.empresa_id}, conciliados={self.movimientos_conciliados})>"


# === AGREGAR RELACIONES A MODELOS EXISTENTES ===
# Nota: Estas relaciones se deben agregar a los modelos existentes en mysql_models.py

"""
# Agregar a EmpresaContribuyente:
