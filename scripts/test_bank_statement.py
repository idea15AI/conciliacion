#!/usr/bin/env python3
"""
Test del procesador de estados de cuenta bancarios
"""
import requests
import json
import sys
...
def compare_extraction_methods(pdf_file):
    ...
    try:
        ...
        resultado_normal = processor.procesar_estado_cuenta(pdf_bytes)
        
        # Método 2: Extracción con tablas
        print("🧪 Método 2: Extracción con tablas")
        resultado_tablas = processor.procesar_con_tablas(pdf_bytes)
        ...
        movimientos_normal = len(resultado_normal.get('movimientos', []))
        movimientos_tablas = len(resultado_tablas.get('movimientos', []))
        
        print(f"Procesamiento Normal: {movimientos_normal} movimientos")
        print(f"Extracción Tablas:    {movimientos_tablas} movimientos")
        
        # Mostrar columnas encontradas
        estructura_normal = resultado_normal.get('estructura_columnas', {})
        columnas_normal = estructura_normal.get('columnas_encontradas', [])
        print(f"Columnas encontradas (Normal): {columnas_normal}")
...
def main():
    print("🏦 TEST DEL PROCESADOR DE ESTADOS DE CUENTA BANCARIOS")
    print("=" * 90)
    ...
    try:
        # 4. Probar procesador de estados de cuenta
        resultado = test_bank_statement_processor(pdf_file)
        
        # 5. Comparar métodos
        resultados_comparacion = compare_extraction_methods(pdf_file)
